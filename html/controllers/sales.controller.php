...
class ControllerSales{
	...
	static public function ctrCreateSale(){
		...
		if(isset($_POST['sUserName'])){
			...
			if(preg_match('/^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ]+$/', $_POST["sUserName"]) &&
			   preg_match('/^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ]+$/', $_POST["oldSerial"])&&
			   preg_match('/^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ]+$/', $_POST["newSerial"])){

				$table = 'checkout';

				$data = array('old_serial' => $_POST['oldSerial'],'loaner_serial' => $_POST['newSerial'],'s_user' => $_POST['sUserName']);

				$answer = CategoriesModel::mdlAddSale($table, $data);
				// var_dump($answer);$answer = ModelSales::mdlAddSale($table, $data);
